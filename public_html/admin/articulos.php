...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'guardar') {
        ...
        $autorUrl = trim((string) ($_POST['autor_url'] ?? '')) ?: null;
        ...
        if ($titulo === '' || mb_strlen($contenido) < 20) {
            ...
            $sql = 'UPDATE une_articulos SET titulo=:titulo, resumen=:resumen, contenido=:contenido, categoria=:categoria, autor=:autor, autor_url=:autor_url,
                    meta_titulo=:meta_titulo, meta_descripcion=:meta_descripcion, publicado=:publicado, fecha=:fecha';
            $params = [
                ':titulo' => $titulo, ':resumen' => $resumen, ':contenido' => $contenido, ':categoria' => $categoria, ':autor' => $autor, ':autor_url' => $autorUrl,
            ...
            $pdo->prepare(
                'INSERT INTO une_articulos (titulo, slug, resumen, contenido, imagen, categoria, autor, autor_url, meta_titulo, meta_descripcion, publicado, fecha)
                 VALUES (:titulo, :slug, :resumen, :contenido, :imagen, :categoria, :autor, :autor_url, :meta_titulo, :meta_descripcion, :publicado, :fecha)'
            )->execute([
                ':titulo' => $titulo, ':slug' => $slug, ':resumen' => $resumen, ':contenido' => $contenido,
                ':imagen' => $imagenArchivo, ':categoria' => $categoria, ':autor' => $autor, ':autor_url' => $autorUrl, ':meta_titulo' => $metaTitulo,
  ...
  <div class="editor-negocio__layout">
    <div class="editor-negocio__principal">
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <div class="campo"><label for="autor_url">Perfil del autor (LinkedIn u otro)</label><input type="url" id="autor_url" name="autor_url" placeholder="https://www.linkedin.com/in/..." value="<?= e($articulo['autor_url'] ?? '') ?>"></div>
